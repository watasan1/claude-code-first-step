...
公式サイト（[Claude Desktop ダウンロードページ](https://claude.com/download)）から、自分のOSに合ったClaudeデスクトップアプリのインストーラーをダウンロードします。
...
本章では、 `rm`コマンドを`trash`コマンドへ置き換え、安全にファイルやフォルダを削除できます。
...
Claude Codeへファイルやフォルダの削除を依頼する機械は少なくありません。
既定では`rm`コマンドが実行されるため、削除した内容はゴミ箱を経由せず、そのまま削除されます。

そこで、`rm`コマンドを`trash`コマンドへ置き換え、削除したファイルやフォルダがいったんゴミ箱へ移動するよう設定します。
