...
作成したテスト項目に沿って、ブラウザでの動作確認（手動テスト）を行なってください
...
Windowsでは、PowerShellスクリプトを作成、Linuxでは「trash-cli」というパッケージを使うことで、他のOSでも同様の仕組みを作れるそうです。

Claude Code上からファイルやフォルダの削除を依頼することは多いと思います。
Claude Codeがファイルやフォルダを削除するとき、rm コマンドを使います。
このコマンドは強力で、一瞬でファイルやフォルダを削除できてしまいます。

一方で、「ファイルやフォルダを削除する前にいったんゴミ箱に入れて、確認してから完全に削除したい」という要望もあると思います。
それを叶えるのがtrash コマンドです。
...
1. Claude Codeに、テスト用のファイルを作成するよう依頼します（例：「test.txt というファイルを作成してください」）。
作成された test.txt を確認します。
Claude Codeに、そのファイルを削除するよう依頼します（例：「test.txt を削除してください」）。
フォルダから test.txt が消えていることを確認します。
Finderで「ゴミ箱」を開き、test.txt が入っていることを確認します。
フォルダから消えていて、かつゴミ箱に test.txt が入っていれば、Claude Codeからの削除依頼が rm ではなく trash 経由で行われ、完全消去ではなくゴミ箱移動になっていることが確認できます。
