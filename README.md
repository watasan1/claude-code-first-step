...
本章はmacOS環境で動作を確認しています。

Apple標準の `trash` コマンドは、macOS Sequoia (15)以降で利用できます。
Sonoma (14)以前では標準搭載されていないため、後述するHomebrew版を利用してください。

Claude Codeでは、ファイルやフォルダの削除時に通常 `rm` コマンドが使用されます。

本章では、`rm` コマンドを `trash` コマンドへ置き換え、削除したファイルやフォルダがごみ箱へ移動するよう設定します。
